"""Knowledge base catalog helpers shared by admin and Java internal APIs."""
...
class KnowledgeBaseService:
    ...
    async def exists(self, db: AsyncSession, value: str) -> bool:
        ...
        if not normalized:
            return False
        allowed: set[str] = set()
        for option in await self.list_options(db):
            for candidate in (option.value, option.label, option.id, option.code):
                candidate_value = normalize_knowledge_base_name(candidate)
                if candidate_value:
                    allowed.add(candidate_value)
    ...
    async def _fetch_business_knowledge_bases(self, db: AsyncSession) -> list[KnowledgeBaseOption]:
        sql = text(
            ...
            SELECT id::text AS id,
             ...
             WHERE status::text = :normal_status
        ...
        for row in rows:
            ...
            if not name:
                continue
            code = normalize_knowledge_base_name(row.get("code"))
