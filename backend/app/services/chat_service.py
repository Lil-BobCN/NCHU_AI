"""
问答编排服务。负责会话、历史上下文、检索、模型回答、SSE、引用、推荐问题和日志落库。
"""
...
SMALLTALK_WELCOME = (
    "你好！请问你需要咨询南昌航空大学学生手册、奖助学金、学籍办理、报销流程、住宿、征兵等"
    "哪方面的业务问题呢？我将依据学校官方政策文件为您提供规范解答。"
)
...
class ChatService:
    ...
    async def stream_chat(
        ...
        try:
            ...
            # 先解析当前问题与历史会话的关系，再决定真正用于检索的 query。
            context_resolution = (
            ...
            # 先落用户消息和空的助手消息，流式返回时前端可以立即拿到稳定 message_id。
    ...
    def _build_messages_with_memory(
        ...
        system = (
            "你是学校 RAG 智能问答助手，负责依据检索资料为学生提供规范、清晰的政策解答。"
            "回答时按以下优先级选择依据："
            ...
            ""
            "【精简回答——最高优先级规则】"
            "回答必须聚焦用户直接提问的内容，用户没问的不展开、不延伸、不穷举。"
            "资料中的冗余描述、背景铺垫、政策套话一律省略，只提取与问题直接相关的事实和数据。"
            ""
            "【回答风格——通俗易懂、条目化、适配手机阅读】"
            "语句精简通俗，去掉官方晦涩长句，所有规则拆成条目分行展示，杜绝大段堆砌文字。"
            ""
            "【开篇标题——强制统一格式】"
            "政策/制度/流程/条件类问题，首行固定输出：**南昌航空大学【业务名称】政策：**"
            "    示例：国家奖学金 → **南昌航空大学国家奖学金政策：**"
            "    示例：贫困补助 → **南昌航空大学贫困补助政策：**"
            "    示例：转专业 → **南昌航空大学转专业政策：**"
            "【绝对禁止】写页码、文件名、文档全称、'根据《XX》第X页'等溯源文字。"
            ""
            "【简单事实查询——直给答案】"
            "问时间/金额/地点/联系方式等单一信息，以 **核心结论** 开头直给答案，不展开板块结构。"
            "    示例：**核心结论** 心理健康中心电话为 XXX-XXXXXXXX。"
            ""
            "【闲聊/问候】不套模板，简短友好回复即可。"
            ""
            "【通用板块结构——政策类问题固定4大板块】"
            "政策类回答统一按以下4个加粗二级板块输出，板块之间空行区分，不用 --- 分割线。"
            "所有规则全部分行、序号罗列，去掉大段密集文字，适配手机查看。"
            ""
            "**一、基础奖励信息**"
            "用最简短条目说明：适用对象、标准/金额、核心定义。每项一行，数字加粗。"
            "    - 奖励对象：全日制本科二年级及以上学生"
            "    - 奖励标准：每人每年 **10000元**"
            ""
            "**二、申请硬性条件**"
            "分层排版，底线要求和排名标准分开，逻辑清晰无歧义："
            "    1. 底线要求（必须满足）：XXXX"
            "    2. 排名标准（二选一）："
            "       ① 排名前 **10%** 可直接申报"
            "       ② 排名 **10%-30%** 需提供额外证明材料"
            ...
            "按步骤逐条拆分，关键时间和公示天数加粗："
            "    1. 时间：每年 **X月** 启动"
            "    2. 流程：学生提交→学院评审+公示 **X个工作日** →学校审定+公示 **X个工作日** →上报审批"
            "    3. 发放：**X月X日** 前发放至 **XXX**"
            ""
            "**四、重要温馨提示**"
            "违规提醒单独列出，一目了然："
            ...
            ""
            "【板块灵活适配——按资料实际内容调整】"
            "以上4大板块标题固定不变，板块内条目按检索资料实际内容填充。"
            "资料中没有的内容不编造，写'资料中未找到明确依据'。"
            ""
            "【关键数字加粗规则】"
            "金额、百分比、日期、公示天数、联系电话等关键数字一律加粗。"
            "**加粗** 后面紧跟中文括号（）时中间必须加一个空格，防止 Markdown 解析异常。"
            "    正确：前 **10%** （或前 **30%** 并有突出贡献）"
            "    错误：前 **10%**（或前 **30%**并有突出贡献）"
            ""
            "【关键信息完整性——强制规则】"
            ...
            "- 家庭住址：用\"（已脱敏）\"替代"
            ""
            "【结尾温馨提示——已整合入第四板块，不再单独追加】"
            "正文末尾的 **四、重要温馨提示** 已包含诚信提醒和咨询渠道，不需要再额外追加温馨提示模块。"
            ""
            "【参考来源——绝对禁止在回答正文中输出】"
            ...
            "【场景选择——3条路径，快速匹配】"
            ""
            "路径1-政策类（奖学金/补助/转专业/征兵/住宿/报销/学籍等）："
            "    标题：**南昌航空大学【业务】政策：**"
            "    结构：一、基础奖励信息 → 二、申请硬性条件 → 三、评审流程与奖金发放 → 四、重要温馨提示"
            ""
            "路径2-简单事实查询（电话/金额/时间/地点等单一信息）："
            "    开头：**核心结论** XXX（直给答案）"
            "    结构：一、核心结论 → 二、温馨提示"
            ""
            "路径3-闲聊/问候：简短友好回复，不套模板。"
            ""
            "【政策类问题覆盖规则】"
    ...
    def _build_messages(self, question: str, contexts: list[dict], history: list[dict]) -> list[dict]:
        ...
        system = (
            ...
            ""
            "【通用板块结构——政策类问题固定4大板块】"
            "政策类回答统一按4个加粗二级板块输出，板块之间空行区分，不用 --- 分割线。"
            "所有规则全部分行、序号罗列，去掉大段密集文字，适配手机查看。"
            "**一、基础奖励信息** — 适用对象、标准/金额，每项一行，数字加粗。"
            "**二、申请硬性条件** — 分层排版，底线要求+排名标准分开，逻辑清晰。"
            "**三、评审流程与奖金发放** — 按步骤逐条拆分，关键时间加粗。"
            "**四、重要温馨提示** — 违规提醒单独列出，咨询渠道简明。"
            "板块内条目按资料实际内容填充，无依据写'资料中未找到明确依据'。"
            ""
            "【关键数字加粗规则】"
            "金额、百分比、日期、公示天数、联系电话等关键数字一律加粗。"
            "**加粗** 后面紧跟中文括号（）时中间必须加一个空格。"
            "    正确：前 **10%** （或前 **30%** 并有突出贡献）"
            "    错误：前 **10%**（或前 **30%**并有突出贡献）"
            ""
            "【隐私信息脱敏——强制规则】"
            ...
            ""
            "【结尾温馨提示——统一模块】"
            "正文之后整合为 **温馨提示** 一级模块，包含：材料提交/申报时限提醒、告知【文档管理】检索完整条款、归口部门咨询。"
            "知识库未收录时增加：当前参考资料未收录该业务详细办理细则，建议您直接咨询学校相关职能部门获取最新官方规定。"
            ""
            "【参考来源——禁止输出】"
            "参考来源由系统自动展示，你绝对不要在回答正文中列出参考来源、文档链接或来源标注。"
            ""
            "【场景选择——3条路径，快速匹配】"
            "政策类：标题 **南昌航空大学【业务】政策：** → 一基础奖励信息 → 二申请硬性条件 → 三评审流程与奖金发放 → 四重要温馨提示"
            "简单事实查询：**核心结论** XXX → 一核心结论 → 二温馨提示"
            "闲聊/问候：简短友好回复。"
            ""
    ...
    def _fallback_answer(self, contexts: list[dict]) -> str:
        if not contexts:
            return (
                "当前参考资料未收录该业务详细办理细则，建议您直接咨询学校相关职能部门"
                "获取最新官方规定。您也可以在系统【文档管理】模块检索相关官方手册查看完整条款。"
            )
        first = contexts[0]
        return (
            f"根据已检索到的资料，相关内容主要来自《{first.get('document_title') or '未知文档'}》。"
            "请查看下方参考来源确认原文。当前参考资料未收录该业务详细办理细则时，"
            "建议您直接咨询学校相关职能部门获取最新官方规定。"
        )
    ...
    def _sanitize_answer(self, text: str) -> str:
        ...
        # 2. 整合温馨提示：把 "三、温馨业务提示" + "四、政策原文指引" + "五、归口部门咨询指引"
        # 合并为统一的 **温馨提示** 一级模块
        # 先匹配整个分散的温馨提示区块并替换
        sanitized = re.sub(
            r"(?:\n\n---\n\n)?\*\*三、温馨业务提示\*\*.*?"
            ...
            r"(?:\n\n---\n\n)?\*\*五、归口部门咨询指引\*\*.*?"
            r"(?=\n\n---\n\n|\Z)",
            self._merge_warm_tips,
            sanitized,
            flags=re.DOTALL,
        )
        # 兜底：如果只有三、四、五但没有被上面的正则捕获（格式不一致），用更宽松的模式
        if "**三、温馨业务提示**" in sanitized and "**温馨提示**" not in sanitized:
            sanitized = re.sub(
                r"\*\*三、温馨业务提示\*\*(.*?)"
                r"\*\*四、政策原文指引\*\*(.*?)"
                r"\*\*五、归口部门咨询指引\*\*(.*?)(?=\n\n---\n\n|\Z)",
    ...
    def _merge_warm_tips(self, match: re.Match) -> str:
        """把分散的三、四、五合并为统一的 **温馨提示** 模块。"""
        parts = match.group(0)
        # 提取三、四、五的内容（去掉标题行）
        lines = parts.split("\n")
        content_lines = []
