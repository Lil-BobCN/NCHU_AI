...
# ============================================================
# 反馈管理（Feedback）— V2 内部接口，从 feedback.py 迁移至 /internal/rag 前缀
# 使用 Sa-Token JWT 认证（get_current_user_from_sa_token），解决旧 feedback.py
# 走 /api/v1 端口80 → rag_backend 容器 JWT 密钥不匹配导致的 401 问题
# ============================================================

_ALLOWED_FEEDBACK_ERROR_TYPES = {
    "answer_wrong",
    "citation_wrong",
    "off_topic",
    "incomplete",
...
class FeedbackCreateRequest(BaseModel):
    ...
    @classmethod
    def validate_feedback_error_type(cls, value: str) -> str:
        normalized = value.strip()
        if normalized not in _ALLOWED_FEEDBACK_ERROR_TYPES:
            raise ValueError("unsupported feedback error type")
        return normalized

    @field_validator("description")
    @classmethod
    def normalize_feedback_description(cls, value: str) -> str:
        return value.strip()
...
async def submit_answer_feedback(
    ...
    """提交答案反馈（内部接口，由 Java 后端 FeedbackProxyController 调用）。"""
...
async def cancel_answer_feedback(
    ...
    if feedback.retrieval_log_id:
        retrieval_log = await db.scalar(
            select(RetrievalLog).where(_retrieval_log_id_filter(feedback.retrieval_log_id))
        )
        if retrieval_log is not None:
            quality = dict(retrieval_log.answer_quality or {})
