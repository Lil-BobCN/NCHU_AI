...
async def list_conversations(
    ...
    return ok(
        {
            "items": [serialize_conversation(item) for item in conversations],
            "page": page,
            "page_size": page_size,
            "total": total or 0,
        }
...
async def list_messages(
    ...
    return ok([serialize_message(item) for item in rows.scalars()])
...
def serialize_message(item: ConversationMessage) -> dict:
    return {
        "id": str(item.id),
