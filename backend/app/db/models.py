"""
ORM 数据模型。定义文档、任务、切片、会话、检索日志、反馈和评测等表结构。
"""
...
class Document(Base, TimestampMixin):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, server_default=func.uuid_generate_v4())
    java_attach_id: Mapped[int | None] = mapped_column(BigInteger, unique=True, nullable=True)
    java_doc_id: Mapped[str | None] = mapped_column(String(128), nullable=True)
    publish_dept_id: Mapped[str | None] = mapped_column(String(128), nullable=True)
    owner_user_id: Mapped[str | None] = mapped_column(String(128), nullable=True)
    visible_in_chat: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default="true")
    publish_scope: Mapped[str] = mapped_column(String(32), nullable=False, server_default="dept")
    allowed_dept_ids: Mapped[list[str] | None] = mapped_column(ARRAY(Text), nullable=True)
    allowed_user_ids: Mapped[list[str] | None] = mapped_column(ARRAY(Text), nullable=True)
