  ...
  <AppShell>
    <div class="page">
            ...
            <tbody>
              <tr v-for="doc in documents" :key="doc.id">
                ...
                <td class="actions">
                  ...
                  <button class="danger" @click="remove(doc.id)">删除</button>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <ConfirmDialog
        ...
        :subject-label="deleteConfirm.subjectLabel"
        :detail="deleteConfirm.detail"
        :busy="Boolean(batchBusy)"
...
function remove(id: string) {
  ...
  const name = document?.title || document?.file_name || '该文档'
  deleteConfirm.value = {
    kind: 'single',
    documentId: id,
    title: '确认删除文档',
    message: `将删除“${name}”，删除后不可恢复。`,
    subjectLabel: '1 个文档',
    detail: '取消不会影响当前文档，确认删除后将刷新文档列表。'
  }
}
...
function batchDeleteDocuments() {
  ...
  deleteConfirm.value = {
    kind: 'batch',
    title: '确认删除文档',
    message: `将删除已选中的 ${selectedDocumentIds.value.size} 个文档，删除后不可恢复。`,
    subjectLabel: `${selectedDocumentIds.value.size} 个文档`,
    detail: '取消不会清空当前选择，确认删除后将刷新文档列表。'
  }
}
...
async function confirmDelete() {
  ...
  if (!pending) return
  deleteConfirm.value = null
  if (pending.kind === 'single' && pending.documentId) {
    await deleteDocument(pending.documentId)
    return
  }
  if (!hasSelectedDocuments.value || batchBusy.value) return
